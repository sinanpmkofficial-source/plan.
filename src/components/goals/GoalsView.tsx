...
export default function GoalsView() {
  ...
  return (
    <div className="max-w-4xl mx-auto pt-6 space-y-8 animate-in fade-in duration-200 text-foreground pb-10">
      ...
      <div className="bg-kbd-bg p-1.5 rounded-md flex gap-1 inline-flex text-xs md:text-sm font-bold shadow-none select-none">
        <button onClick={() => setActiveTab('active')} className={`py-1.5 px-4 rounded-md text-center transition-all cursor-pointer ${ activeTab === 'active' ? 'bg-black text-white shadow-none' : 'text-neutral-550 hover:text-black hover:bg-neutral-200/50' }`}>
          Active ({totalActive})
        </button>
        <button onClick={() => setActiveTab('completed')} className={`py-1.5 px-4 rounded-md text-center transition-all cursor-pointer ${ activeTab === 'completed' ? 'bg-black text-white shadow-none' : 'text-neutral-550 hover:text-black hover:bg-neutral-200/50' }`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 min-h-[300px]">
        <AnimatePresence mode="popLayout" initial={false}>
          {filteredGoals.length === 0 ? (
            <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="col-span-full flex flex-col items-center justify-center p-12 border border-dashed border-divider rounded-lg bg-card-bg/20 text-neutral-400 text-center min-h-[250px]">
              <Target className="w-8 h-8 stroke-1 mb-2.5 text-neutral-400" />
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        {goal.description && <p className="text-xs md:text-sm text-neutral-500 mt-1.5 break-words leading-relaxed font-semibold">{goal.description}</p>}
                      </div>
                      <div className="flex items-center gap-1 shrink-0">
                        <button onClick={() => handleStartEdit(goal)} className="p-1.5 hover:bg-neutral-200 hover:text-black text-neutral-400 rounded-md transition-colors cursor-pointer" title="Edit Goal"><Pencil className="w-4 h-4" /></button>
                        <button onClick={() => setGoalToDelete(goal.id)} className="p-1.5 hover:bg-neutral-200 hover:text-black text-neutral-400 rounded-md transition-colors cursor-pointer" title="Delete Goal"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </div>
                    {totalMilestones > 0 && (
                      <div className="space-y-1.5">
                        <div className="flex justify-between items-center text-xs font-bold text-neutral-400"><span className="uppercase tracking-wider">Milestones progress</span><span className="text-foreground">{progressPct}%</span></div>
                        <div className="h-1.5 w-full bg-neutral-200 rounded-sm overflow-hidden"><div className="h-full bg-black transition-all duration-300" style={{ width: `${progressPct}%` }} /></div>
                      </div>
                    )}
                    {totalMilestones > 0 ? (
                      <div className="space-y-2 pt-2">
                        ...
                        <div className="space-y-1 max-h-[160px] overflow-y-auto pr-1">
                          {goal.milestones.map((m) => (
                            <button key={m.id} onClick={() => toggleMilestone(goal.id, m.id)} className="w-full flex items-start gap-2.5 p-2 rounded-md hover:bg-neutral-100 text-left transition-colors cursor-pointer group">
                              {m.completed ? (
                                <span className="w-4 h-4 mt-0.5 border border-black bg-black text-white rounded flex items-center justify-center text-[10px] font-black shrink-0">✓</span>
                              ) : (
                                <span className="w-4 h-4 mt-0.5 border border-neutral-300 hover:border-black rounded block shrink-0" />
                              )}
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-md">
          <div className="bg-card-bg border border-neutral-200 p-6 rounded-2xl max-w-md w-full shadow-none animate-in zoom-in-95 duration-120 max-h-[90vh] overflow-y-auto text-foreground">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowAddModal(false)} className="p-1.5 hover:bg-neutral-200 rounded-md text-neutral-500"><X className="w-4 h-4" /></button>
            </div>
            <form onSubmit={handleSubmitGoal} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs text-neutral-500 uppercase tracking-wider font-extrabold">Goal Title</label>
              ...
              <div className="space-y-1">
                <label className="text-xs text-neutral-500 uppercase tracking-wider font-extrabold">Description (Optional)</label>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <label className="text-xs text-neutral-500 uppercase tracking-wider font-extrabold">Milestones</label>
                ...
                <div className="space-y-2">
                  {newMilestones.map((m, idx) => (
                    <div key={idx} className="flex gap-2">
                      ...
                      {newMilestones.length > 1 && <button type="button" onClick={() => handleRemoveMilestoneField(idx)} className="p-2 border border-input-border text-neutral-400 hover:text-black hover:border-black rounded-md cursor-pointer shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>}
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-md">
          <div className="bg-card-bg border border-neutral-200 p-6 rounded-2xl max-w-md w-full shadow-none animate-in zoom-in-95 duration-120 max-h-[90vh] overflow-y-auto text-foreground">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setEditingGoalId(null)} className="p-1.5 hover:bg-neutral-200 rounded-md text-neutral-500"><X className="w-4 h-4" /></button>
            </div>
                ...
                <div className="space-y-2">
                  {editMilestones.map((m, idx) => (
                    <div key={idx} className="flex gap-2">
                      ...
                      {editMilestones.length > 0 && <button type="button" onClick={() => handleRemoveEditMilestoneField(idx)} className="p-2 border border-input-border text-neutral-400 hover:text-black hover:border-black rounded-md cursor-pointer shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>}
                    </div>
                  ))}
                </div>
              </div>
